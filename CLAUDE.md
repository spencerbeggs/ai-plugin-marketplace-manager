...
- `pnpm schema:build` / `pnpm schema:check` — regenerate / verify the committed
  `schemas/1.0/{input,output}.json` (`schema:check` runs first in `ci:test`).
...
- Effect Schemas are the source of truth; `schemas/1.0/{input,output}.json`
  are generated by the schemastore CLI and **drift-checked** by
  `pnpm schema:check` in `ci:test` — run `pnpm schema:build` after schema
  changes, don't hand-edit. The `$schema` URLs are one `HostedSchema`
