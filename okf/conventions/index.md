...
* [Keep the Action Contract in Sync](keep-the-action-contract-in-sync.md) - Editing an input or output means editing three places, an Effect Schema change means rebuilding the versioned JSON Schema documents, and a schema label bump means updating the prose that quotes the URL.
* [Keep the entry-point guard and the env strip together](entry-point-guard-and-env-strip-are-a-pair.md) - Keep each entry point's GITHUB\_ACTIONS guard and vitest.setup.ts's env strip paired; dropping either lets a test run a real phase on a runner.
